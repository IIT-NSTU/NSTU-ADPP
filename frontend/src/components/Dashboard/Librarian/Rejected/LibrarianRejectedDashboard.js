...
const LibrarianRejectedDashboard = () => {
    let navigate = useNavigate();
    const [certificateData, setCertificateData] = useState([])
    const getData = async () => {
        axios.get('http://localhost:8000/api/provisional-rejected-list-by-librarian/')
            .then(res => {
                console.log('hukka')
                let newArr = []
                res.data.map((v, i) => (
...
export default connect(mapStateToProps, {})(LibrarianRejectedDashboard)