...
const CourierApprovedDashboard = () => {
    let navigate = useNavigate();
    const [certificateData, setCertificateData] = useState([])
    const getData = async () => {
        axios.get('http://localhost:8000/api/provisional-accepted-list-by-courier/')
            .then(res => {
                console.log('hukka')
                let newArr = []
                res.data.map((v, i) => (
    ...
    const columns = [
        ...
        {
            title: "phone number", field: "phone", filterPlaceholder: "Filter by phone", align: 'center'
        },
    ];

    const userDetails = (data) => {
        console.log("in details", data.roll)
        navigate(`/courier/student-details/${data.roll}`)
    }
...
export default connect(mapStateToProps, {})(CourierApprovedDashboard)