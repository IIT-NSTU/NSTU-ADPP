...
import './CourierRequestedDashboard.css';

const CourierRequestedDashboard = () => {
    let navigate = useNavigate();
    const [certificateData, setCertificateData] = useState([])
    const getData = async () => {
        axios.get('http://localhost:8000/api/provisional-applied-list-for-courier/')
            .then(res => {
                console.log('hukka')
                let newArr = []
                res.data.map((v, i) => (
...
export default connect(mapStateToProps, {})(CourierRequestedDashboard)